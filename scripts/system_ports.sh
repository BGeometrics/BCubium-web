...
NGINX=$(grep " listen \[::\]:" /etc/nginx/sites-available/default | cut -d "]" -f2 | cut -d ":" -f2 | awk {'print $1'} | head -n1)

echo "{\"bitcoin_port\":\"$BITCOIN_PORT\", \"lnd_port\":\"$LND_PORT\", \"btcrpcexplorer_port\":\"$BTCRPCEXPLORER_PORT\", \"rtl_port\":\"$RTL_PORT\", \"tor_port\":\"$TOR_PORT\", \"wireguard_port\":\"$WIREGUARD_PORT\", \"openvpn_port\":\"$OPENVPN_PORT\", \"nginx_port\":\"$NGINX\"}"
